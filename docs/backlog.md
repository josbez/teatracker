...
**Afgerond: 18 van de 29 items.** Alle front-end items behalve de "Later"-parkeerlijst zijn klaar.
Wat resteert: de 3 **backend** big-bets (QA-1, FE-2, QA-3) en de 8 bewust geparkeerde "Later"-features.
...
- **Sprint 2 — front-end** (PR #5): FE-4, A11Y-3, A11Y-5, UX-2, UX-3, MO-1, MO-4, QA-4, QA-5
...
```
   ...
   M │  ✅ A11Y-1 Datumcontrast           │  ✅ A11Y-3 Focus-trap in sheet
     ...
     │  ✅ FE-4 Rijkere statistieken      │  ⬜️ FE-6 Brouwparameters
     │  ✅ MO-1 FAB → × morph             │  ⬜️ FE-7 Subscores / 5-sterren
     │                                   │  ⬜️ FE-5 Wishlist · ⬜️ FE-8 Foto
     │ ─────────────────────────────────┼───────────────────────────────
   L │  FILL-INS  (tussendoor)           │  LATER  (lage prioriteit)
     │  ✅ UX-2 Scroll-fade soortenrij    │  ⬜️ FE-8 Foto bij thee
     │  ✅ MO-4 Haptische feedback        │  ⬜️ FE-11 Engelse vertaling (i18n)
     │  ✅ A11Y-5 Tablist-semantiek       │
...
### 🟢 Quick wins · hoge/middel impact, lage effort — ✅ allemaal klaar
...
| 9 | ✅ | FE-4 | **Rijkere statistieken** (favoriete soort in de stats-band) | Feature | M | L-M | UX |
| 10 | ✅ | MO-1 | **FAB → × morph** bij openen sheet (continuïteit) | Motion | M | L | Motion |

### 🔵 Big bets · hoge impact, meer effort

| # | Status | ID | Item | Scope | Impact | Effort | Bron |
|---|:------:|----|------|-------|:------:|:------:|------|
| 11 | ⬜️ | QA-1 | **Betrouwbare schrijfbevestiging** — Apps Script CORS/JSONP-respons i.p.v. blind "opgeslagen" *(backend)* | Data/Bug | H | M | QA |
| 12 | ⬜️ | FE-2 | **Bewerken van entries** (`update`-actie in backend + edit-sheet) *(backend)* | Feature | H | M-H | Diego |
| 13 | ⬜️ | QA-3 | **Datum als ISO opslaan** (enabler voor echte sortering/tijdlijn) *(backend)* | Data | M-H | M | QA |
...
### ⚪ Fill-ins · lage effort — ✅ allemaal klaar

| # | Status | ID | Item | Scope | Impact | Effort | Bron |
|---|:------:|----|------|-------|:------:|:------:|------|
| 16 | ✅ | UX-2 | Scroll-fade/affordance op de soort-filterrij | UX | L-M | L | UX |
| 17 | ✅ | MO-4 | Haptische feedback (`navigator.vibrate`) bij opslaan/verwijderen/rating | Motion | L | L | Mara |
| 18 | ✅ | UX-3 | Warmere lege/onboarding-staat eerste keer | UX | M | L-M | UX |
| 19 | ✅ | A11Y-5 | Rating-filter correcte `role="group"` + `aria-pressed` | A11y | L | L | QA |
| 20 | ✅ | QA-5 | Duplicaat-hint bij gelijke naam (niet-blokkerend) | Bug | L | L | QA |
| 21 | ✅ | QA-4 | Refocus-reload temmen (60s-debounce + geen her-animatie bij ongewijzigde data) | Bug/Perf | L-M | L | Diego |
...
- **Bugs / dataintegriteit:** ✅ QA-2, QA-4, QA-5 · ⬜️ QA-1, QA-3
- **Toegankelijkheid:** ✅ A11Y-1, A11Y-2, A11Y-3, A11Y-4, A11Y-5
- **UX / visueel:** ✅ UX-1, UX-2, UX-3, FE-4
- **Motion:** ✅ MO-1, MO-2, MO-4 · ⬜️ MO-3
- **Infra:** ✅ INF-1
- **Nieuwe features:** ✅ FE-1, FE-3, FE-4 · ⬜️ FE-2, FE-5, FE-6, FE-7, FE-8, FE-9, FE-10, FE-11

---

## 6. Aanbevolen volgorde

- **Sprint 1 + 2 (front-end):** ✅ afgerond — alle quick wins + fill-ins, plus A11Y-3 focus-trap en MO-2 drag-to-dismiss.
- **Next (backend):** QA-1 Schrijfbevestiging · FE-2 Bewerken · QA-3 ISO-datum. *Deze drie vergen Apps Script-werk; zet die code eerst in de repo, dan zijn ze te doen.*
- **Later:** Dark mode, FLIP, brouwparameters/subscores, wishlist, export, foto, i18n — op basis van wat gebruikers daarna het meest vragen.

> Alle afgeronde items zijn **puur front-end** (één `index.html`); `main` → Netlify deployt automatisch.
> De open big-bets raken de **Apps Script-backend** achter de Google Sheet — die code staat nog niet in versiebeheer.
...
| Datum | Release | Items |
|-------|---------|-------|
| 2026-06-27 | PR #5 | Sprint 2 (front-end): FE-4 stats · A11Y-3 focus-trap · A11Y-5 filter-semantiek · UX-2 scroll-fade · UX-3 onboarding · MO-1 FAB-morph · MO-4 haptics · QA-4 refocus · QA-5 dup-hint |
| 2026-06-27 | PR #4 | MO-2 drag-to-dismiss · copy-fix sheettitel ("proeven" → "toevoegen") |
